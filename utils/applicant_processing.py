...
def check_blacklist(applicant_name, alliance_tag, blacklist):
    ...
    if applicant_name in blacklist.get('names', []):
        app_logger.warning(f"Applicant '{applicant_name}' is in the blacklist.")
        return True
    if alliance_tag in blacklist.get('tags', []):
        app_logger.warning(f"Alliance tag '{alliance_tag}' is in the blacklist.")
        return True
    return False
...
def click_applicant_checkbox(index, device_id, base_x, base_y, y_increment):
    ...
    try:
        ...
        app_logger.exception(f"Error clicking checkbox at index {index}: {e}")
        return False
